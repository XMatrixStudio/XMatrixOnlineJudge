...
function getCookie(name) {
    ...
    $.post(
      ...
      function(data) {
        if (data.state == 'success') {
          ...
          if(data.why == 'IS_JUDGING')alert('正在评测中,请不要重复提交');
        }
      });
  }  //评测模块
  ...
  function dealGrade(data) {
    ...
    $('#j_Ltime').html('最后提交：' + data.last_time);
    $('#j_Rtime').html('运行时间：' + data.run_time + 'ms');
    $('#j_men').html('最高分数：' + data.grade_max);
    $('#j_info1').html(data.grade_1);
    $('#j_info2').html(data.grade_2);
    $('#j_info3').html(data.grade_3);
    $('#j_info4').html(data.grade_4);
  ...
  //-----------------------------------------------------------------
  if (data.grade_1 === grade_1m) {
    $('#j_test1')
    .html(
      '编译测试： ' + data.grade_1 + '/' + grade_1m +
  ...
  //-----------------------------------------------------------------
  if (data.grade_2 === grade_2m) {
    $('#j_test2')
    .html(
      '标准测试： ' + data.grade_2 + '/' + grade_2m +
      ' <span class="glyphicon glyphicon-ok" style="float:right;"></span>');
  ...
  //-----------------------------------------------------------------
  if (data.grade_3 === grade_3m) {
    $('#j_test3')
    .html(
      '随机测试： ' + data.grade_3 + '/' + grade_3m +
      ' <span class="glyphicon glyphicon-ok" style="float:right;"></span>');
  ...
  //-----------------------------------------------------------------
  if (data.grade_4 === grade_4m) {
    $('#j_test4')
    .html(
      '内存测试： ' + data.grade_4 + '/' + grade_4m +
      ' <span class="glyphicon glyphicon-ok" style="float:right;"></span>');
