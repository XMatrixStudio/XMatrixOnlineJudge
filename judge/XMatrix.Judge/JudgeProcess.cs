...
namespace XMatrix.Judge {
  public class JudgeProcess {
    ...
    public bool DoJudge() {
      ...
      judge_collection.ReplaceOne(Builders<JudgeCase>.Filter.Eq("_id", judge_case._id), judge_case);
      return true;
    }
    ...
    private void JudgeCPP14() {
      ...
      judge_case.details = new BsonDocument[2];
      ...
      if (standard_test != null) {
        ...
        for (int i = 0; i < standard_test.GetValue("num"); i++) {
          ...
          bash.Command(cmd);
          var compare = new Compare($"{Config.IOPath}{judge_case.pid}/in{i}.txt", $"{Config.IOPath}{judge_case.pid}/std{i}.txt", $"{Config.SandboxPath}out{i}.txt");
        ...
        judge_case.details[1].Add(new BsonElement("grade", Array.Find(problem_case.test, x => x.GetValue("name") == "standard-test").GetValue("val").ToInt32() * grade / standard_test.GetValue("num").ToInt32()));
      }
      judge_case.grade = judge_case.details[0].GetValue("grade").ToInt32() + judge_case.details[1].GetValue("grade").ToInt32();
      judge_case.judging = false;
    }

    private void JudgeC11() {
      return;
    }
  }
}
